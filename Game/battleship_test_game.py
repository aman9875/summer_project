...
def calc_max_prob():
	count=0
	max_matrix=[]
	neighbour_score=[0 for x in range(0,100)]
	max_val = 0
	max_index = [0,0]
	for i in range(0,10):
		for j in range(0,10):
	...
	for i in range(count):
		...
		y=max_matrix[i][1];
		if is_exists(x-1,y):
			neighbour_score[i]=neighbour_score[i]+calc_neighbour_score(x-1,y)
		if is_exists(x,y-1):
			neighbour_score[i]=neighbour_score[i]+calc_neighbour_score(x,y-1)
		if is_exists(x+1,y):
			neighbour_score[i]=neighbour_score[i]+calc_neighbour_score(x+1,y)
		if is_exists(x,y+1):
			neighbour_score[i]=neighbour_score[i]+calc_neighbour_score(x,y+1)
		if(neighbour_score[i]>m):
			m=neighbour_score[i]
			m_x=x
			m_y=y
